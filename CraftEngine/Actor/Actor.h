...
namespace Craft
{
	...
	public:
		...
		//액터 제거 함수
		void Destroy(); //모아두었다가 한 번에 삭제처리

		//게임(엔진) 종료 함수.
		void QuitGame();

		//Getter/Setter
		inline bool HasBeganPlay() const { return hasBeganPlay; } //리턴 할 때 첫글자를 소문자로해야 무한루프에 안걸림
	...
	protected: //확장을 염두에 두고 있을 때
		...
		// 낮은 위치에서는 weak를 사용해야하며 상호참조를 방지해야한다.
