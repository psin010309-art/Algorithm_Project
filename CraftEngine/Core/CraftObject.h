...
namespace Craft
{
	...
	public:
		...
		//타입을 질문하는 함수
		//이 함수를 사용하기 위해서는 T타입이 static TypeId 함수를 가져야함.
		template<typename T>
		bool IsTypeOf() const
		{
			return Is(T::TypeId());
		...
		//U 타입을 T타입으로 변환하게 할 것.(원본을 목표하는 타입으로)
		template<typename T, typename U>
		//U타입의 어떤 객체를 받아서 T타입으로 형변환하는 캐스트함수 생성.
		std::shared_ptr<T> Cast(const std::shared_ptr<U>& object)
		{
			//예외처리
			if (!object) return nullptr;

			//object의 실제 타입이 T(또는 T의 파생 타입)인지 확인 후 형변환(casting)
			//다운캐스팅 U타입인 object가 T타입이 맞는지 묻고 아니면 가벼운 형변환.
			if (object->Is(T::TypeId()))
			{
				//가벼운 형변환을 통해 반환
				//static_pointer_cast는 런타임 타입 검증을 하지 않기 떄문에
				//상대적으로 가벼움.
...
public:                                                                  \
    ...
    {                                                                    \
       /*현재 계층에서 일단 비교 후 , 타입이 다르면 부모 계층까지 검색*/ \
